...
- startup now prebuilds metadata only through the bounded dense startup tier, so the shipping runtime path avoids whole-planet metadata allocation on very large planets while still allowing async high-LOD metadata misses during traversal
...
- resident metadata storage now uses dense slabs indexed by `(lod, face, x, y)` through the bounded startup tier plus sparse `HashMap` residency above that tier, while still storing compact bounds/metrics plus cached same-LOD neighbors so `ChunkKey` and the base surface class are reconstructed on access without re-deriving topology neighbors on the selection hot path
...
- [x] Date: 2026-03-22
- [x] Result summary: `cargo test` now passes, and `./scripts/build_rust.sh` builds successfully. The worker path supports async render payload submission, hybrid dense-plus-sparse metadata residency that removes whole-planet startup spikes on large planets, cached same-LOD neighbors inside those entries so selection stops reconstructing them on every access, bounded dense startup metadata prebuild, async asset-group snapshot preparation, queue-side supersession of older overlapping jobs, and epoch-based stale-result dropping before install. Collision-only copies are no longer precomputed for every render payload; they are derived lazily on the commit lane only for chunks that actually enter the bounded physics set. Headless/runtime logs still expose worker, metadata, and sparse-residency counters alongside the existing queue and scratch metrics.
