...
- Current diagnostics are per-frame counters. After the startup activation burst settles, `phase4_gpu_tile_upload_bytes` and `phase4_gpu_material_binds` return to `0` while `phase4_active_gpu_render_chunks` and `phase4_canonical_meshes` continue to report the live displaced state.
...
- [x] Upload and commit observations: `cargo test` passed `84/84`, `./scripts/build_rust.sh` succeeded, `./scripts/run_godot.sh --headless --quit-after 5` and `./scripts/run_godot.sh --headless res://scenes/main_300km.tscn --quit-after 8` loaded cleanly, and `./scripts/profile_window_modes.sh` reported stable GPU residency across window modes with `avg_phase4_active_gpu_render_chunks=172.0000`, `avg_phase4_canonical_meshes=9.0000`, `avg_render_tile_mib=4.018784`, and `avg_deferred_upload_mib=0.000000` for both `small_window` and `fullscreen_native`. The settled per-frame Phase 04 upload/bind counters returned to `0.000000` / `0.0000` once residency stabilized, which matches the shipped counter semantics.
