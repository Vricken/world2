...
- The main scenes now enable the GPU path by default, while the explicit debug inspector property `PlanetRoot.debug_force_server_pool_render_backend = true` can still switch back to the legacy CPU backend for direct comparison.
...
- [x] Deviations: To keep direct comparisons possible, the CPU mesh backend still exists behind `RenderBackendKind::ServerPool`, but by Phase 06 it is quarantined behind the explicit debug-only inspector property `PlanetRoot.debug_force_server_pool_render_backend`. The main scenes continue to validate the shipped GPU path by default.
