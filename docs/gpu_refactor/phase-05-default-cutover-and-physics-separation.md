...
- Shipped default backend: `RuntimeConfig::default()` and `PlanetRoot` now default to `RenderBackendKind::GpuDisplacedCanonical`. By Phase 06 the only remaining legacy escape hatch is the explicit debug-only property `PlanetRoot.debug_force_server_pool_render_backend`; setting it to `true` forces the legacy `ServerPool` CPU mesh path for comparison.
