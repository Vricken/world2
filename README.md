...
- Phase 06 visibility and LOD selection in `rust/src/runtime/pipeline/selection.rs`, including lazy chunk-metadata caching, horizon-first visibility traversal, projected-error split/merge hysteresis, neighbor LOD delta normalization, separate render/physics active sets, and budgeted commit/upload deferral metrics.
...
- Phase 08 conservative collision commit path in `rust/src/runtime/pipeline/commit.rs`, including `PhysicsServer3D` static-body residency, concave shape refresh for near-camera chunks, bounded physics pooling, and explicit RID teardown on shutdown.
