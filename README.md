...
Phase 15 strategy-layer refinement plus GPU refactor Phases 01-03 for a Godot + Rust (godot-rust/gdext) planet runtime.
...
The probe reports the Phase 01, Phase 02, and Phase 03 control-plane metrics directly, including `selection_reference_height_px`, `target_render_chunks`, `hard_render_chunk_cap`, `avg_selected_candidates`, `avg_refinement_iterations`, `avg_selection_cap_hits`, `avg_render_residency`, `avg_render_residency_evictions`, `avg_render_tile_mib`, `avg_render_tile_pool_slots`, `avg_render_tile_pool_active_slots`, `avg_render_tile_pool_free_slots`, `avg_render_tile_eviction_ready_slots`, `avg_selected_render_starved`, `avg_selected_render_starvation_failures`, `avg_selected_render_starvation_frames`, and `fullscreen_lod_bias=none`. The debug-only Project Setting `world2/debug/lod_viewport_height_override` remains available for manual experiments, but the shipped selector no longer depends on live viewport height to keep fullscreen demand stable while the tile path is brought online behind the existing CPU mesh backend.
