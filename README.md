...
- Phase 10 scene-root rebasing in `rust/src/lib.rs`, including camera-state recovery from render-relative coordinates, physics-tick root-node origin shifts for child gameplay nodes, collision-contact rebase deferral for camera-owned `CharacterBody3D` controllers, interpolation reset on actual rebases, and headless logging for origin rebases and transform rebind counts.
