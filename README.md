...
- Phase 06 visibility and LOD selection in `rust/src/runtime/pipeline/selection.rs`, including sampled per-chunk min/max height metadata, horizon-first visibility traversal with no near-surface hard disable, projected-error split/merge hysteresis, cached split-ancestor bookkeeping, capped near-camera physics residency, and budgeted commit/upload deferral metrics.
...
- Phase 07 configurable metadata prebuild and payload policy in `rust/src/runtime/core.rs`, including `metadata_precompute_max_lod`, `payload_precompute_max_lod`, and default startup metadata prebuild through LOD 5 with lazy metadata fallback above that window.
...
- Phase 08 conservative collision commit path in `rust/src/runtime/pipeline/commit.rs`, including `PhysicsServer3D` static-body residency, concave shape refresh for a capped near-camera chunk set, reduced commit-path cloning, bounded physics pooling, and explicit RID teardown on shutdown.
...
- Phase 09 threaded render payload generation in `rust/src/runtime/workers/payloads.rs`, including persistent Rust worker threads, deterministic request/result ordering, single-lane commit ownership, reusable per-worker scratch buffers for sampling/mesh/packing/slope work, and explicit queue/wait/allocation-style metrics.
