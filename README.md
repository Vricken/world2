...
- PlanetRoot tool-time editor integration in `rust/src/lib.rs`, including exported `planet_radius`, `frustum_culling_enabled`, and `keep_coarse_lod_chunks_rendered` inspector properties, runtime reconstruction from editor-authored values, and a simple in-editor sphere preview that matches the configured planet radius.
...
- Fly debug controller in `scripts/player/fly_controller.gd` and `scenes/main.tscn`, with WASD + Space/Shift flight, mouse look, Up/Down speed scaling over the default spawn outside the planet, and a `20000`-unit far clip on the main camera.
