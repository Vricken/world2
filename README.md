...
Phase 15 strategy-layer refinement plus GPU refactor Phases 01-06 for a Godot + Rust (godot-rust/gdext) planet runtime.
...
- GPU refactor Phase 04 GPU-displaced canonical rendering in `rust/src/runtime/data.rs`, `rust/src/runtime/core.rs`, `rust/src/runtime/pipeline/commit.rs`, `rust/src/runtime/tests.rs`, `rust/src/lib.rs`, `shaders/terrain_gpu_chunk.gdshader`, and `scenes/main.tscn`, including shared canonical `ArrayMesh` ownership per surface class, pooled per-chunk `ShaderMaterial` plus fixed-size `ImageTexture` tile bindings, `RenderingServer` instance overrides and conservative custom AABBs, backend-aware upload accounting that charges compact tile payloads instead of CPU mesh bytes, runtime diagnostics for Phase 04 tile uploads/material binds/active GPU chunks/canonical meshes, and the retained legacy backend comparison path now quarantined behind `PlanetRoot.debug_force_server_pool_render_backend`.
- Parameterized layered terrain in `rust/src/geometry.rs` and `RuntimeConfig`, including shared 3D planet-space continent/hill/mountain/detail sampling for metadata, render payload workers, tests, and asset placement; render material tiles now encode `[height_norm, slope_hint, moisture, land_mask]`, and `shaders/terrain_gpu_chunk.gdshader` blends beach/grass/upland/rock/snow from those channels.
- Simple visual sea rendering in `rust/src/runtime/pipeline/commit.rs`, using one server-managed two-sided opaque water sphere slightly above `planet_radius + sea_level_meters`; water is visual-only in v1, has no physics body, and is rebound on origin shifts separately from chunk residency.
- GPU refactor Phase 05 default cutover and physics separation in `rust/src/runtime/data.rs`, `rust/src/runtime/workers/payloads.rs`, `rust/src/runtime/pipeline/selection.rs`, `rust/src/runtime/pipeline/commit.rs`, `rust/src/runtime/core.rs`, `rust/src/runtime/tests.rs`, `rust/src/lib.rs`, `scripts/profiling/perf_probe.gd`, and `docs/gpu_refactor/phase-05-default-cutover-and-physics-separation.md`, including `RuntimeConfig::default()` and `PlanetRoot` now defaulting to `RenderBackendKind::GpuDisplacedCanonical`, render payload requests that only build/store `ChunkRenderTilePayload` on the shipped GPU path, collision-only CPU mesh retention for desired or active physics chunks, payload-cached conservative GPU custom AABBs reused during commit/rebind, reused `PackedByteArray` staging with bulk float-tile copies instead of per-float main-thread repacking, explicit `collision_residency`/`collision_residency_bytes` metrics alongside render/tile residency, and a debug fallback that still allows `PlanetRoot.debug_force_server_pool_render_backend = true` to force the legacy server-pool mesh path for comparison.
- GPU refactor Phase 06 hardening, cleanup, and acceptance in `rust/src/runtime/data.rs`, `rust/src/runtime/strategy.rs`, `rust/src/runtime/tests.rs`, `rust/src/lib.rs`, `scenes/main.tscn`, `README.md`, and `docs/gpu_refactor/`, including removal of the shipped-path viewport-height override plumbing, final `fullscreen_lod_bias=none` diagnostics on the reference-height selector, an explicitly debug-named server-pool fallback property, and the recorded acceptance benchmark for the shipped GPU tile runtime.
...
- PlanetRoot tool-time editor integration in `rust/src/lib.rs` and `scenes/main.tscn`, including exported `planet_radius`, `terrain_height_amplitude`, `sea_level_meters`, `water_enabled`, `hill_strength`, `mountain_strength`, `mountain_frequency`, `atmosphere_height` as a planet-radius fraction, `frustum_culling_enabled`, and `keep_coarse_lod_chunks_rendered` inspector properties, runtime reconstruction from editor-authored values, a synced `PlanetAtmosphere` child that follows the configured radius and effective shell thickness, and a simple in-editor sphere preview that matches the configured planet radius.
...
- Fly debug controller in `scripts/player/fly_controller.gd` and `scenes/main.tscn`, with WASD + Space/Shift flight, mouse look, Up/Down speed scaling, runtime-derived spawn distance outside the configured atmosphere shell, runtime-synced camera near/far clipping that follows current camera distance plus the atmosphere proxy bounds while capping far/near depth ratio, and a runtime HUD for atmosphere/precision diagnostics.
...
The probe reports the final Phase 01-06 control-plane metrics directly, including `selection_reference_height_px`, `target_render_chunks`, `hard_render_chunk_cap`, `avg_selected_candidates`, `avg_refinement_iterations`, `avg_selection_cap_hits`, `avg_render_residency`, `avg_render_residency_evictions`, `avg_gpu_tile_upload_mib`, `avg_gpu_material_binds`, `avg_active_gpu_render_chunks`, `avg_canonical_render_meshes`, `avg_render_tile_mib`, `avg_render_tile_pool_slots`, `avg_render_tile_pool_active_slots`, `avg_render_tile_pool_free_slots`, `avg_render_tile_eviction_ready_slots`, `avg_collision_residency`, `avg_collision_residency_mib`, `avg_selected_render_starved`, `avg_selected_render_starvation_failures`, `avg_selected_render_starvation_frames`, and `fullscreen_lod_bias=none`. In settled profiles the per-frame GPU upload/bind counters fall back toward `0` once tile residency stabilizes, while the active-GPU, canonical-mesh, and render-tile residency counters continue to describe the live displaced path. The scripted probe stays far enough from the surface that the near-camera physics set usually remains inactive, so `avg_collision_residency` can stay at `0` during these runs even though the collision path is still exercised by the Rust test suite. The shipped selector now uses only the fixed reference-height path, and the only remaining legacy backend escape hatch is the explicit debug inspector property `PlanetRoot.debug_force_server_pool_render_backend`.
