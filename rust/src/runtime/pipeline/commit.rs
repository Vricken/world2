...
impl PlanetRuntime {
    ...
    pub(crate) fn should_commit_to_servers(&self) -> bool {
    ...
    pub(crate) fn current_render_rids(&mut self, key: ChunkKey) -> Option<(Rid, Rid)> {
        let rid_state = self.ensure_rid_state(key);
    ...
    pub(crate) fn take_current_render_entry(&mut self, key: ChunkKey) -> Option<RenderPoolEntry> {
        let rid_state = self.ensure_rid_state(key);
    ...
    pub(crate) fn ensure_commit_staging(
        &self,
    ...
    pub(crate) fn render_transform_for_chunk(&self, chunk_origin_planet: DVec3) -> Transform3D {
        relative_transform(
            chunk_origin_planet,
